...
bool MainFlow::ReadFile(std::string filename, json& jsonfile)
{
...
void MainFlow::ProcessJson(json& jsonfile, std::list<Rectangle>& rectlist)
{
...
bool MainFlow::DetermineIfAlreadyProcessed(std::string rect2, std::string thisname)
{
	/*
	...
	*/
	...
	//Naming conventions of this assignment. All rectangles added from MainFlow are done so by number. Parse out the number of the current rectangle.
...
void MainFlow::FindIntersections(std::list<Rectangle>& originalRectangleDefinition, std::list<Rectangle> generatedRectangleDefinition, int counter)
{
...
*/
bool MainFlow::PerformExercise(const std::string& filename)
{
	json jsonfile;
